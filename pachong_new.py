...
class GetBookMessage:
    ...
    def draw_base_list(self, url):
        ...
        listt = soup.find('div', class_="book-img-text").find_all('li')
        for x in listt:
            self.pic_url = 'https:' + x.find('div', class_='book-img-box').find('a').find('img')['src']
            self.book_name = x.find('div', class_='book-mid-info').find('h2').text.strip()  # 书名
            self.author = x.find('div', class_='book-mid-info').find('p', class_='author').find(class_='name').text.strip()  #
            self.main_type = x.find('div', class_='book-mid-info').find('p', class_='author').find('a', class_='').text.strip()  # 小说的主类型
            self.second_type = x.find('div', class_='book-mid-info').find('p', class_='author').find('a', class_='go-sub-type').text.strip()  # 小说的副类型
            self.book_status = x.find('div', class_='book-mid-info').find('p', class_='author').find('span').text.strip()  # 小说更新的状态
            self.brief_intro = x.find('div', class_='book-mid-info').find('p', class_='intro').text.strip()
            self.latest_chapter = x.find('div', class_='book-mid-info').find('p', class_='update').find('a').text.strip()
    ...
    def dict_data(self):
        create_time = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime());  # 当前爬取时间
        update_time = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime());  # 当前爬取时间
        counts = round(random.uniform(1.0,1000.0),2)
        data = {
